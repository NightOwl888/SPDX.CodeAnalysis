...
namespace SPDX.CodeAnalysis.Tests
{
    public class LicenseHeaderMustBeCorrectFormatTestDriver : FsmlAnalyzerTest<Verifier>
    {
        ...
        public LicenseHeaderMustBeCorrectFormatTestDriver(string fsmlXml, IRootPathNormalizer normalizer, CodeLanguage language, bool suppressLocation = false, string topLevelDirectoryName = "LICENSES.HEADERS")
            : base(fsmlXml, normalizer, language, topLevelDirectoryName)
        {
        ...
        protected override DiagnosticAnalyzer CreateCSharpAnalyzer()
        {
            ...
            return new LicenseHeaderMustBeCorrectFormat(cacheManager, TagValueScannerProvider.Instance, LicenseHeaderScannerProvider.Instance, licenseAnalyzerOptions);
        }
